...
It will come as not surprise that the LA is frequently violated by invoking improper function calls, namely calls from a lower layer to an upper one, disrupting the acyclic schema of the architecture. This usually occurs to very large open-source projects such as the Linux Kernel or the FreeBSD projects. For instance, the Linux Kernel 4.19 codebase consists of 25,900 C source code files and 25,500 C header files, spanning 20.3 million SLOC. 
